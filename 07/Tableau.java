...
public class Tableau {
    ...
    public void addValue(Integer value) {
        if (smallest == null) {
            ...
            Integer bumpValue = addToRow(smallest, value);
            Cell rowPointer = smallest;
            
            if (bumpValue == null) {
                return;
            } else {
                while (rowPointer.below != null) {
                    bumpValue = addToRow(rowPointer.below, bumpValue);
                    if (bumpValue == null) {
                        return;
                    }
                    rowPointer = rowPointer.below;
                }
                rowPointer.below = new Cell(bumpValue); //down link
                rowPointer.below.above = rowPointer; //above link
            }
        }
    }       
    ...
    protected Integer addToRow(Cell curr, int value) {
        int bumpValue = 0;
        if (curr.value >= value) {
            bumpValue = curr.value;
            curr.value = value;
            return bumpValue;
        } else {
            while (curr.right != null){
                if (curr.right.value >= value) {
                    bumpValue = curr.right.value;
                    curr.right.value = value; // right link
                    curr.right.left = curr; //left link
                    if (curr.above != null) {
                        curr.right.above = curr.above.right; //above link
                        curr.above.right.below = curr.right; // below link
                    }
